...
def _snapshot_to_prompt_text(snapshot: dict, mode: OverviewMode) -> str:
    ...
    lines = [
        ...
        "",
        "=== LIQUIDITÄT ===",
        f"Ist-Saldo (aktueller Kontostand, gebucht): {_format_euro(snapshot['current_balance'])}",
        f"Projektion Monatsende (Ist-Saldo + geplante Buchungen Monat): {_format_euro(snapshot['available_funds_month'])}",
        f"Projektion Gesamt (Ist-Saldo + alle geplanten Buchungen): {_format_euro(snapshot['available_funds_total'])}",

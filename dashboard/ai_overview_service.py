...
def _snapshot_to_prompt_text(snapshot: dict, mode: OverviewMode) -> str:
    ...
    lines = [
        ...
        "",
        "=== LIQUIDITÄT ===",
        f"Ist-Saldo (gebucht): {_format_euro(snapshot['current_balance'])}",
        f"Projektion Monat (Ist-Saldo + geplante Buchungen bis Monatsende): {_format_euro(snapshot['available_funds_month'])}",
        f"Projektion Gesamt (Ist-Saldo + alle geplanten Buchungen, Stunden und Auslagen): {_format_euro(snapshot['available_funds_total'])}",
